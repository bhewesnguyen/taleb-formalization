...
## Final statuses (v0.2.6)

- Final build: **passes** — clean `lake build` exit 0 (28 modules, 2743 jobs, no warnings); `verify.py` PASS 125/8/16 (149 checked); also from a fresh extraction.
- Axiom checks: **all 270 project constants** (149 public theorem/instance, 32 defs, structure type, 19 Lean-generated, 69 internal) depend only on `propext`, `Classical.choice`, `Quot.sound`; no `sorryAx`; no project `axiom`; ledger valid under the shared validator; all 107 citations (106 distinct declarations) exist.
- Verifier regression: **14/14 fixtures** as expected from re-extracted sources and a pristine `.lake/build`; ledger probes 12/12 rejected.
- Mathematics added (v0.2.6): reverse-Weibull (α > 0) probability measure with cdf identity and max-stability for every n; positive affine pushforward with its cdf rule; `gumbelLaw`/`frechetLaw`/`reverseWeibullLaw` and the law of iid maxima as measure equalities (T061 discharged). Backlog: 2 discharged (T060, T061), 9 partial, 4 reuse, 91 missing, 34 source-check, 15 model-needed, 3 empirical.
- Unresolved: unified GEV parametrisation and EVT densities (T062), domains of attraction (T011/T093), stable-law existence for α < 2 (T007) and Cauchy identification (T046), probabilistic Property 5.1 for nonnegative sums (T029), subexponentiality of concrete laws, positive/measurable RV and Karamata, Pareto moment integrals.
...
location/scale laws, T061 discharged → `83ef501` v0.2.6 final evidence and manifest (the packaged tree) → this commit
(v0.2.6 deliverables).
