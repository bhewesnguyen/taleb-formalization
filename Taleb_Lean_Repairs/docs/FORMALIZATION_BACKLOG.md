...
Delivery scope: Ratio-only slowly/regularly varying predicates with closure lemmas, examples (log, constants, real powers) and the RV-iff-normalised-SV equivalence. The negative-constant diagnostic shows that positivity is not implied.

Remaining obligations: Positive measurable convention and its relationship to the ratio predicates. Uniform convergence, Potter bounds and Karamata are T026, T027 and T002.
...
Delivery scope: Exact-Pareto conditional and excess laws as equalities of probability measures (threshold law, excess law with global cdf and strict survival), conditional mean and mean excess for alpha > 1, and, for 0 < alpha <= 1, divergence of the conditional raw first moment as an extended nonnegative integral (AuditPareto.lintegral_id_cond_paretoMeasure_eq_top: no separate divergence theorem for the excess law is exported). Source: eq. (2.10) printed p. 18 (PDF 32) states the general tail integral identity for nonnegative X (proof displayed p. 260 / PDF 274). The delivered slice realizes its consequences for the Pareto law only.
...
Delivery scope: Gaussian slice only: Mathlib's gaussianReal mu (2 sigma^2) realizes stableS1Expr 2 beta mu sigma, zero scale included. This reuses Mathlib's law rather than constructing a new one.

Remaining obligations: Existence of a probability law with the S1 characteristic function for every 0 < alpha < 2, including alpha = 1 with beta != 0, and the zero-scale Dirac case for those alpha.
...
Delivery scope: Definition of the nonnegative self-convolution class and preservation of a finite tail exponent under that hypothesis. Both subexponentiality and the finite exponent are hypotheses, not conclusions.

Remaining obligations: Any concrete subexponential law, the equivalent n-fold tail characterizations, equivalence with the standard definition, and whether subexponential laws must have a finite exponent.
...
**P1 / partial / MOM**. Printed pp. 80-86; PDF anchor p. 94; source: 4.4.2-4.4.4.

Compute Gaussian, Student, and Pareto absolute moments and MAD/STD ratios. Delivered (v0.2.8, exact-Pareto slice, AuditRepairs/ParetoLaw.lean): for Mathlib's paretoMeasure L alpha (density alpha L^alpha x^(-alpha-1) on [L, inf), the book's p. 86 display) the p-th moment as an extended nonnegative integral equals alpha L^p/(alpha - p) for p < alpha and is +inf for p >= alpha (including p = alpha), Integrable (x^p) iff p < alpha, the real integral alpha L^p/(alpha - p) in the finite range (value 1 at p = 0), and equality of absolute and raw moments on the positive support. Delivered (v0.2.10, centered Pareto slice, AuditRepairs/ParetoMoments.lean): mean alpha L/(alpha - 1) for alpha > 1 (and L < mean), the centered mean absolute deviation E(abs(X - m)) = 2 L (alpha - 1)^(alpha - 2)/alpha^(alpha - 1) for alpha > 1 (via abs t = 2 max t 0 - t, the zero mean of X - m, and the positive part as the upper-tail integral nu(Ioi m) * E(X - m given X > m) through the Stage B threshold law at K = m), the variance alpha L^2/((alpha - 1)^2 (alpha - 2)) for alpha > 2 through Mathlib's variance and variance_eq_sub, the standard deviation L/(alpha - 1) sqrt(alpha/(alpha - 2)), and the STD/MAD ratio alpha^(alpha - 1/2)/(2 sqrt(alpha - 2) (alpha - 1)^(alpha - 1)) of eq. (4.14), with MAD > 0 proved before dividing. Sanity values at L = 1, alpha = 3: mean 3/2, variance 3/4, MAD 4/9.
...
Delivered (law_theorem, actual_law_constructed, source_reviewed): `AuditPareto.momentLintegral`, `AuditPareto.momentLintegral_eq`, `AuditPareto.momentLintegral_of_lt`, `AuditPareto.momentLintegral_eq_top`, `AuditPareto.ae_nonneg_rpow_paretoMeasure`, `AuditPareto.integrable_rpow_paretoMeasure_iff`, `AuditPareto.integral_rpow_paretoMeasure`, `AuditPareto.integral_rpow_zero_paretoMeasure`, `AuditPareto.integral_abs_rpow_paretoMeasure`, `AuditPareto.paretoMean`, `AuditPareto.integral_id_paretoMeasure`, `AuditPareto.integrable_id_paretoMeasure`, `AuditPareto.lt_paretoMean`, `AuditPareto.integral_sub_const_cond_paretoMeasure`, `AuditPareto.setIntegral_eq_measureReal_mul_integral_cond`, `AuditPareto.integral_posPart_paretoMeasure`, `AuditPareto.paretoMAD`, `AuditPareto.paretoMAD_algebra`, `AuditPareto.integral_abs_sub_mean_paretoMeasure`, `AuditPareto.paretoMAD_pos`, `AuditPareto.integrable_sq_paretoMeasure`, `AuditPareto.memLp_two_id_paretoMeasure`, `AuditPareto.integral_sq_paretoMeasure`, `AuditPareto.paretoVariance`, `AuditPareto.variance_id_paretoMeasure`, `AuditPareto.paretoStd`, `AuditPareto.paretoStd_eq`, `AuditPareto.paretoStd_div_paretoMAD`, `AuditPareto.paretoMean_one_three`, `AuditPareto.paretoVariance_one_three`, `AuditPareto.paretoMAD_one_three`.

Delivery scope: Exact-Pareto raw and absolute moments against the pinned Mathlib law: finite exactly for p < alpha with value alpha L^p/(alpha - p), divergent (extended integral = top) for p >= alpha including the boundary, integrability equivalence, zeroth moment 1. Source: p. 86 (PDF 100) gives the same density and a standard deviation consistent with the p = 1, 2 values. Since v0.2.10 also the centered Pareto slice: mean, centered mean absolute deviation, variance (Mathlib's variance), standard deviation and the STD/MAD ratio (4.14), as integral theorems under the actual law, plus the L = 1, alpha = 3 sanity values. Source: p. 86 (PDF 100), eq. (4.14) and the displayed standard deviation.

Remaining obligations: Gaussian and Student absolute moments and their MAD/STD ratios (4.4.2-4.4.3), the general MAD/STD comparison across families, and the scale conventions of each family. The Pareto ratio (4.14) is delivered, and a Pareto example does not close the multi-distribution target.
...
**P1 / partial / MOM**. Printed pp. 95-98; PDF anchor p. 109; source: 5.1.

Prove finite qth moment for q<alpha and infinite qth moment for q>alpha; isolate q=alpha. Delivered (v0.2.8, exact-Pareto slice, AuditRepairs/ParetoLaw.lean): for paretoMeasure L alpha the closed-form strict survival (1 below L, (L/x)^alpha from L on, value 1 at x = L) and cdf (0 below L, 1 - (L/x)^alpha, value 0 at x = L), no atom at the endpoint, finite log-tail exponent alpha of the actual survival function, and the moment threshold exactly: finite for q < alpha, extended integral = top for q >= alpha (the boundary q = alpha is infinite for the exact law, whose slowly varying factor is constant).

Hypotheses and gaps: q>=0; nonnegative law with regularly varying tail of index -alpha; boundary requires an integral test on L. The general regularly varying statement keeps its statement-review gate: the boundary q = alpha depends on the slowly varying factor L(x) and must be specified (integral test on L) before the general theorem can be stated.
...
Prove the heavier regularly varying tail dominates a sum of nonnegative variables with positive weights (event-inclusion bounds, no independence needed). Delivered (v0.2.3): the formula-level two-term identity AuditTails.two_power_tail and its symmetric form AuditTails.two_power_tail_min (exponent min(alpha1, alpha2)) in AuditRepairs/Foundations.lean. Delivered (v0.2.7, AuditRepairs/WeightedSums.lean): the random-variable statement in the finite log-tail-exponent interface: for pointwise nonnegative X, Y on a probability space, positive weights a, b and survival functions with exponents alpha, beta, the survival function of aX + bY has exponent min(alpha, beta) (AuditProbability.hasFiniteTailExponent_weightedSum), also for the pushforward laws with measurable coordinates (hasFiniteTailExponent_survival_map_weightedSum). Route: the event inclusions {X > t/a}, {Y > t/b} subset {aX + bY > t} subset {X > t/(2a)} union {Y > t/(2b)} and the analytic rules HasFiniteTailExponent.comp_const_mul (positive rescaling), .max and .add (exponent min) and .of_le_of_le (squeeze). No independence.

Hypotheses and gaps: Nonnegative variables and positive weights are required: with signed dependent summands, X = E Z and Y = -2 E Z give 2X + Y = 0, so the unrestricted printed Property 5.1 is false (SOURCE_GATES G18). The displayed limit under Property 5.1 (p. 99) is printed without the minus sign; the delivered lemmas state the corrected version. The binary probabilistic finite-log-exponent statement was delivered in v0.2.7 for pointwise nonnegative coordinates and positive weights (no independence). The original unequal-index regular-variation reading of the target is a separately recorded stronger child (see remaining obligations): a finite log-tail exponent does not imply regular variation (S(t) = t^(-alpha) exp(epsilon sin log t) has exponent alpha but S(e^pi t)/S(t) oscillates), so exponent equality and the ratio statement are different theorems. Finite-family, almost-sure-nonnegativity and extended-real-exponent extensions are recorded follow-ups, not missing premises of the delivered theorem.
...
Delivery scope: Gaussian half: gaussianReal realizes the S1 expression at alpha = 2 and the stable convolution bridge is instantiated with actual Gaussian laws. Cauchy: expression-level specialization only.

Remaining obligations: A Cauchy probability law with its characteristic function (none in the pinned Mathlib), nonnegative scale including Dirac, and its instantiation of the bridge.
...
Delivery scope: Prerequisite only: convolution powers, their characteristic functions and the conditional stable sum/convolution bridge. No sample-average law is delivered.

Remaining obligations: Pushforward under division by n, the average scale n^(1/alpha - 1), the alpha = 1 skew-location correction under rescaling, and finite-mean convergence.
...
Delivered (formula_proved, law_theorem, source_reviewed, discharged): `AuditExtremes.maxLeEvent`, `AuditExtremes.minGtEvent`, `AuditExtremes.minGeEvent`, `AuditExtremes.maxLtEvent`, `AuditExtremes.maxRV`, `AuditExtremes.minRV`, `AuditExtremes.measure_iInter_preimage`, `AuditExtremes.measure_maxLeEvent`, `AuditExtremes.measure_minGtEvent`, `AuditExtremes.measure_minGeEvent`, `AuditExtremes.measure_maxLtEvent`, `AuditExtremes.measure_maxLeEvent_of_forall_eq`, `AuditExtremes.measure_minGtEvent_of_forall_eq`, `AuditExtremes.measureReal_maxLeEvent_of_forall_eq`, `AuditExtremes.measureReal_minGtEvent_of_forall_eq`, `AuditExtremes.measure_maxLeEvent_of_map_eq`, `AuditExtremes.measure_minGtEvent_of_map_eq`, `AuditExtremes.measurable_maxRV`, `AuditExtremes.measurable_minRV`, `AuditExtremes.cdf_map_maxRV`, `AuditExtremes.measureReal_map_minRV_Ioi`, `AuditExtremes.measureReal_map_minRV_Ioi_eq_one_sub_cdf`.

Delivery scope: Generic probability-law theorems: the cdf of the maximum equals (cdf nu)^n and the survival of the minimum equals (nu(x, inf))^n = (1 - cdf nu x)^n for independent measurable coordinates with common law nu, plus event-level product and power identities for all four threshold conventions.

Remaining obligations: None within the family. Named EVT laws are T061 (discharged in v0.2.6). Domains of attraction are T011 (general formulation) and T062 (Frechet via regular variation, Gaussian/Gumbel normalisers). Density identifications of the EVT laws are supplemental obligation S002 (docs/SUPPLEMENTAL_OBLIGATIONS.md), not T062 as earlier text said.

### T061 - GEV families as probability measures

**P1 / discharged / EVT**. Printed pp. 172-173; PDF anchor p. 186; source: 9.1.

Construct global Gumbel, Frechet and reverse-Weibull CDFs and prove validity. Delivered: the three standardised laws as probability measures on R built from their distribution functions through Stieltjes functions (Gumbel and Frechet xi > 0 in v0.2.5, reverse-Weibull alpha > 0 with upper endpoint 0 in v0.2.6), exact cdf identities, analytic max-stability of each formula, and the positive location/scale pushforward affineLaw nu mu sigma = nu.map (z -> mu + sigma z) with cdf (affineLaw nu mu sigma) x = cdf nu ((x - mu)/sigma), giving gumbelLaw, frechetLaw and reverseWeibullLaw with the book's G((x - b_n)/a_n) distribution functions and, for iid coordinates, the law of the maximum as an equality of measures: gumbelLaw (mu + sigma log n) sigma, frechetLaw xi mu (sigma n^xi), reverseWeibullLaw alpha mu (sigma n^(-1/alpha)) (AuditRepairs/ExtremeValueLaws.lean, AuditRepairs/ExtremeValueAffine.lean).

Hypotheses and gaps: Correct piecewise support; scale>0; shape domains; right continuity and endpoint limits. Delivered with: Frechet xi > 0 and reverse-Weibull alpha > 0 as explicit hypotheses, positive scale sigma > 0 for every location/scale statement (negative scale is out of scope), right-continuity and the endpoint limits 0 at -inf and 1 at +inf are proved and are exactly what the Stieltjes construction needs. The support-correct piecewise CDFs are in fact globally continuous for positive shape (Frechet tends to 0 from the right at its lower endpoint 0, reverse-Weibull to 1 from the left at its upper endpoint 0), and global continuity is also proved (AuditEVTLaws.continuous_frechetCDF, continuous_reverseWeibullCDF, v0.2.7); the totalised-power caveat 0^(-1/xi) = 0 concerns only the unguarded frechetFormula (frechetFormula_zero), not frechetCDF. Scale: affineLaw and its probability instances accept any real sigma, while the cdf rule and the maximum-law theorems require sigma > 0 (a negative scale reverses the inequalities and is not covered). The unified GEV parametrisation in xi with the xi -> 0 Gumbel limit is not stated (supplemental obligation S001, docs/SUPPLEMENTAL_OBLIGATIONS.md): the three families are separate laws.

Delivered (formula_proved, conditional_law_theorem, law_theorem, actual_law_constructed, source_reviewed, discharged): `AuditTails.frechetFormula`, `AuditTails.frechetCDF`, `AuditTails.gumbelCDF`, `AuditTails.frechetFormula_zero`, `AuditTails.frechet_formula_maxstable`, `AuditTails.frechet_cdf_maxstable`, `AuditTails.gumbel_maxstable`, `AuditExtremes.measureReal_maxLeEvent_frechet`, `AuditExtremes.measureReal_maxLeEvent_gumbel`, `AuditEVTLaws.gumbelStieltjes`, `AuditEVTLaws.gumbelMeasure`, `AuditEVTLaws.gumbelMeasure_isProbabilityMeasure`, `AuditEVTLaws.cdf_gumbelMeasure_apply`, `AuditEVTLaws.cdf_map_maxRV_gumbel`, `AuditEVTLaws.cdf_map_maxRV_pi_gumbel`, `AuditEVTLaws.frechetStieltjes`, `AuditEVTLaws.frechetMeasure`, `AuditEVTLaws.frechetMeasure_isProbabilityMeasure`, `AuditEVTLaws.cdf_frechetMeasure_apply`, `AuditEVTLaws.cdf_map_maxRV_frechet`, `AuditEVTLaws.cdf_map_maxRV_pi_frechet`, `AuditEVTLaws.reverseWeibullCDF`, `AuditEVTLaws.reverseWeibullCDF_monotone`, `AuditEVTLaws.reverseWeibullCDF_continuousWithinAt_Ici`, `AuditEVTLaws.reverseWeibullCDF_tendsto_atBot`, `AuditEVTLaws.reverseWeibullCDF_tendsto_atTop`, `AuditEVTLaws.continuous_frechetCDF`, `AuditEVTLaws.continuous_reverseWeibullCDF`, `AuditEVTLaws.reverseWeibullStieltjes`, `AuditEVTLaws.reverseWeibullMeasure`, `AuditEVTLaws.reverseWeibullMeasure_isProbabilityMeasure`, `AuditEVTLaws.cdf_reverseWeibullMeasure_apply`, `AuditEVTLaws.reverseWeibullCDF_maxstable`, `AuditEVTLaws.cdf_map_maxRV_reverseWeibull`, `AuditEVTLaws.cdf_map_maxRV_pi_reverseWeibull`, `AuditEVTLaws.affineLaw`, `AuditEVTLaws.affineLaw_isProbabilityMeasure`, `AuditEVTLaws.cdf_affineLaw`, `AuditEVTLaws.gumbelLaw`, `AuditEVTLaws.frechetLaw`, `AuditEVTLaws.reverseWeibullLaw`, `AuditEVTLaws.cdf_gumbelLaw`, `AuditEVTLaws.cdf_frechetLaw`, `AuditEVTLaws.cdf_reverseWeibullLaw`, `AuditEVTLaws.map_maxRV_gumbelLaw`, `AuditEVTLaws.map_maxRV_frechetLaw`, `AuditEVTLaws.map_maxRV_reverseWeibullLaw`.

Delivery scope: All three standardised EVT laws (Gumbel, Frechet xi > 0, reverse-Weibull alpha > 0 with upper endpoint 0) as probability measures with exact cdf identities and max-stability instantiated for the constructed laws including product-space iid realizations. The positive affine pushforward API with its cdf rule, the three location/scale families with distribution functions G((x - mu)/sigma), and the law of the maximum of iid location/scale coordinates as an equality of probability measures with the parameter rules mu + sigma log n (Gumbel), sigma n^xi (Frechet), sigma n^(-1/alpha) (reverse-Weibull).
...
Hypotheses and gaps: p>0; common positive scale; random alpha>p; integrable mixing or extended expectation. Source correction (v0.2.9, audit of v0.2.8): the second-derivative display below (21.7) on p. 382 reads x0^p * 2/(alpha - 1)^3; the correct value is m_p''(alpha) = 2 p L^p/(alpha - p)^3 for alpha > p, agreeing with the display, as an identity in alpha, only for p = 1 (SOURCE_GATES G20). Convexity in alpha needs p > 0 (negative orders do not inherit it).
...
Delivery scope: Corrected rational excess formula and its positivity in the finite-inverse-moment regime, as algebra.

Remaining obligations: The inverse moment of the gamma law, its finiteness threshold, and the mixture expectation identity that produces the formula.
